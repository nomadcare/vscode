	...
	async handlePrompt(prompt: string, model: string): Promise<void> {
		...
		if (!isFirst) {
			...
			systemOverride = [
				...
				"",
				"⚠️ Не меняй обязательные версии SDK и библиотек, указанные в первоначальном промпте, включая Expo SDK 53 и Expo Go 53, Metro конфиг ^0.82.4, React Native 0.79 и React 19, а также все перечисленные зависимости и devDependencies, если только пользователь явно не попросит обновить их.",
				"",
